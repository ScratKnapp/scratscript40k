...
if CLIENT then
  function ITEM:PaintOver(item, w, h)
    draw.SimpleText(item:GetData('stacks', 1), 'DermaDefault', w - 5, h - 5, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM, 1, color_black)
  end
end
...
ITEM.functions.combine = {
  ...
  OnCanRun = function(firstItem, data) return true end
}
...
ITEM.functions.split = {
  ...
  OnRun = function(item)
    ...
    client:RequestString('Split', 'Please enter how many items you want to split', function(splitStack)
      ...
      if not character:GetInventory():Add(itemUniqueID, 1, {
        stacks = cleanSplitStack
      }) then
        ix.item.Spawn(itemUniqueID, client, nil, angle_zero, {
          stacks = cleanSplitStack
        })
      end
  ...
  OnCanRun = function(item) return item:GetData('stacks', 1) ~= 1 end
}
