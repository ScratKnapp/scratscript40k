...
if CLIENT then
  ...
  function PANEL:Init()
    ...
    number:Dock(FILL)
    number.Paint = function(this, w, h) draw.SimpleText(self.number, "nutDialFont", w / 2, h / 2, color_white, 1, 1) end
  end
  ...
  function PANEL:Init()
    ...
    for i = 1, 5 do
      if i ~= 4 then
        ...
        dot:Dock(LEFT)
        dot:SetWide(30)
        dot.Paint = function(this, w, h) draw.SimpleText(".", "nutDialFont", w / 2, h - 10, color_white, 1, 4) end
      end
    end
  end
  ...
  netstream.Hook("radioAdjust", function(freq, id)
    ...
    if freq then
      for i = 1, 5 do
        if i ~= 4 then adjust.dial[i].number = tonumber(freq[i]) end
      end
    end
  end)
else
...
ix.chat.Register("radio", {
  ...
  GetColor = function(speaker, text) return RADIO_CHATCOLOR end,
  CanHear = function(self, speaker, listener)
    ...
    local freq
    if not CURFREQ or CURFREQ == "" then return false end
