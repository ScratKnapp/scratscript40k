...
-- function PLUGIN:OnContainerRegistered()
...
-- else
-- print("[Helix] Container for '"..k.."' is missing all inventory information!")
...
ix.command.Add("ContainerCreate", {
    ...
    OnRun = function(self, client, model, name, w, h, useKey)
        ...
        timer.Simple(0.5, function()
            hook.Run("SpawnContainer", client, model, name, w, h, useKey)
        end)
    end
})
...
if SERVER then
    ...
    function PLUGIN:SpawnContainer(client, model, name, width, height, usekey)
        ...
        -- container:SetNoDraw(nodraw and nodraw or false)
        container:Spawn()
        -- container:SetNetVar("desc", "")
        container:SetNetVar("width", width)
        container:SetNetVar("height", height)
        container:SetNetVar("useKey", usekey)

        -- container:SetNetVar("openSound", "Pathos.OpenDrawer")
        -- container:SetNetVar("closeSound", "Pathos.CloseDrawer")
        -- container:SetNetVar("lockedSound", "games/ue4/horrorengine/handlerelease04.wav")
    ...
    function PLUGIN:SaveContainer()
        ...
        for _, v in ipairs(ents.FindByClass("ix_container")) do
            if hook.Run("CanSaveContainer", v, v:GetInventory()) ~= false then
                ...
                if inventory then
                    data[#data + 1] = {v:GetPos(), v:GetAngles(), inventory:GetID(), v:GetModel(), v.password, v:GetDisplayName(), v:GetMoney(), v:GetNetVar("width", nil), v:GetNetVar("height", nil), v:GetMaterial(), v:GetSkin(), v:GetColor(), v:GetRenderMode(), v:GetNetVar("desc", ""), v:GetNetVar("useKey", false), v.spawnCategory, v:GetNetVar("openSound", "Pathos.OpenDrawer"), v:GetNetVar("closeSound", "Pathos.CloseDrawer"), v:GetNetVar("lockedSound", "games/ue4/horrorengine/handlerelease04.wav"), v:GetNetVar("multipleUsers", false)}
                end
            else
                local index = v:GetID()
    ...
    function PLUGIN:LoadData()
        ...
        if data then
            for _, v in ipairs(data) do
                ...
                if v[8] then
                    entity:SetNetVar("width", v[8])
                end

                if v[9] then
                    entity:SetNetVar("height", v[9])
                end

                if v[10] then
                    entity:SetMaterial(v[10])
                end
                ...
                if v[12] then
                    entity:SetColor(v[12])
                end

                if v[13] then
                    entity:SetRenderMode(v[13])
                end

                if v[14] then
                    entity:SetNetVar("desc", v[14])
                end

                if v[15] then
                    entity:SetNetVar("useKey", v[15])
                end

                if v[16] then
                    entity.spawnCategory = v[16]
                end

                if v[17] then
                    entity:SetNetVar("openSound", v[17])
                end

                if v[18] then
                    entity:SetNetVar("closeSound", v[18])
                end

                if v[19] then
                    entity:SetNetVar("lockedSound", v[19])
                end

                if v[20] then
                    entity:SetNetVar("multipleUsers", v[20])
                end
...
properties.Add("container_setpassword", {
    ...
    Filter = function(self, entity, client)
        ...
        -- if not gamemode.Call("CanProperty", client, "container_setpassword", entity) then return false end
        return true
    end,
