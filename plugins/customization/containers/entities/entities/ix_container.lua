...
if SERVER then
  ...
  function ENT:SetInventory(inventory)
    if inventory then self:SetID(inventory:GetID()) end
  end
  ...
  function ENT:OpenInventory(activator, t)
    ...
    if inventory then
      ...
      if self:GetLocked() then self.Sessions[activator:GetCharacter():GetID()] = true end
  ...
  function ENT:OnOptionSelected(activator, option, data)
    if option == "search" then
      if self:GetLocked() then
        ...
        if self:GetNetVar("useKey", false) then activator:Hint("The container is locked.") end
      else
