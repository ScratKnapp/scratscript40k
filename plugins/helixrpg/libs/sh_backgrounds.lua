...
function ix.backgrounds.LoadFromDir(directory)
  for _, v in ipairs(file.Find(directory .. "/*.lua", "LUA")) do
    ...
    if PLUGIN then BACKGROUND.plugin = PLUGIN.uniqueID end
...
function ix.backgrounds.Setup(client)
  ...
  if character then
    for k, v in pairs(ix.backgrounds.list) do
      if v.OnSetup then v:OnSetup(client, character:GetBackground(k, false)) end
    end
  end
end
...
do
  ...
  if SERVER then
    ...
    function charMeta:IsHuman()
      ...
      if background == "Human" then
        return true
      elseif background == "Amphii" then
        return true
      elseif background == "Ratling" then
        return true
      elseif background == "Adeptus Mechanicus" then
        return true
      elseif background == "Squat" then
        return true
      elseif background == "Homo-Navigo" then
        return true
      elseif background == "Felinid" then
        return true
      elseif background == "Darksider" then
        return true
      elseif background == "Ogryn" then
        return true
      elseif background == "Longshank" then
        return true
      else
        return false
      end
  ...
  function charMeta:GetRaceBonus(attribute)
    local background = self:GetBackground()
    local bonuses
    if self:GetData("RaceBonus") and attribute == self:GetData("RaceBonus") then return 1 end
    for k, v in pairs(ix.backgrounds.list) do
      if v.name == background then bonuses = v.bonuses end
    end
...
do
  ix.char.RegisterVar("background", {
    ...
    OnValidate = function(self, value, data, client) if value ~= nil then if not isstring(value) then return false, "unknownError" end end end,
    ShouldDisplay = function(self, container, payload) return not table.IsEmpty(ix.traits.list) end
  })

  hook.Add("DoPluginIncludes", "HRPGLoadBackgrounds", function(path, PLUGIN) ix.backgrounds.LoadFromDir(path .. "/backgrounds") end)
end
