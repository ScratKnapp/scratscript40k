...
ITEM:Hook("drop", function(item)
  if item:GetData("equip") then
    ...
    if item.armorclass ~= "helmet" then item.player:SetModel(item.player:GetChar():GetModel()) end
  end
end)
      ...
      if char then
        ...
        if inv then
          ...
          if attData then
            ...
            if not curPrice then curPrice = item.price end
...
function ITEM:RemoveAttachment(id, client)
  ...
  local attachments = self:GetData("outfitAttachments", {})
  if item and attachments[id] then item:OnDetached(client) end
  attachments[id] = nil
  self:SetData("outfitAttachments", attachments)
end
...
ITEM.functions.Equip = {
  ...
  OnCanRun = function(item)
    local client = item.player
    return not IsValid(item.entity) and IsValid(client) and item:GetData("equip") ~= true and hook.Run("CanPlayerEquipItem", client, item) ~= false and item.invID == client:GetCharacter():GetInventory():GetID()
  end
}
...
function ITEM:OnRemoved()
  ...
  if IsValid(owner) and owner:IsPlayer() then if self:GetData("equip") then end end
end
...
function ITEM:GetDescription()
  ...
  if customData.desc then str = customData.desc end
  if customData.longdesc then str = str .. "\n\n" .. customData.longdesc end
  if self.humanOnly then str = str .. "\n\nOnly usable by Humans and Abhumans" end
  if self.race then str = str .. "\nOnly usable by " .. self.race .. " Race" end
  if self:GetData("AP") then str = str .. "\n\nArmor Points: " .. self:GetData("AP") .. "/" .. self:GetData("MaxAP") end
  if self:GetData("Type") then str = str .. "\nArmor Type: " .. self:GetData("Type") end
  if self:GetData("Quality") then str = str .. "\n" .. self:GetData("Quality") .. " Craftmanship" end
  ...
  if self.noUpgrade then str = str .. "\n\n Does not take armor upgrades" end
  local mods = self:GetData("mod", {})
  if mods then
    str = str .. "\n\nModifications:"
...
function ITEM:GetName()
  ...
  local customData = self:GetData("custom", {})
  if customData.name then name = customData.name end
  return name
end
...
ITEM.functions.Clone = {
  ...
  OnRun = function(item)
    ...
    client:requestQuery("Are you sure you want to clone this item?", "Clone", function(text)
      if text then
        ...
        if not inventory:Add(item.uniqueID, 1, item.data) then client:Notify("Inventory is full") end
      end
    end)
    return false
  end,
  OnCanRun = function(item)
    local client = item.player
    return client:GetCharacter():HasFlags("N") and not IsValid(item.entity)
  end
}
...
function ITEM:ApplySpecialQualityBuffs()
  local client = self.player
  local character = client:GetCharacter()
  if self:HasQuality("Fashionable") then character:AddBoost("Fashionable", "appearance", 1) end
  if self:HasQuality("Medicae") then character:AddSkillBoost("Medicae", "medicine", 2) end
  if self:HasQuality("Wraithbone") then character:AddBoost("Wraithbone", "dexterity", 1) end
  if self:HasQuality("Asuryani-Runes") then character:AddSkillBoost("Asuryani-Runes", "warpattunement", 2) end
  if self:HasQuality("Intimidating") then character:AddSkillBoost("Intimidating", "intimidation", 2) end
  if self:HasQuality("Disgusting") then character:AddBoost("Disgusting", "appearance", -1) end
  ...
  if self:HasQuality("Psi-Enhancing") then character:AddSkillBoost("Psi-Enhancing", "warpattunement", 1) end
  ...
  if self:HasQuality("Minor-Runes") then character:AddSkillBoost("Minor-Runes", "warpattunement", 1) end
end
