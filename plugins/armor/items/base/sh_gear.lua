...
ITEM.category = "Gear"
ITEM.skincustom = {}
ITEM:Hook("drop", function(item) if item:GetData("equip") then item:SetData("equip", nil) end end)
ITEM.functions.RemoveUpgrade = {
  ...
  OnCanRun = function(item)
    ...
    if char:HasFlags("2") then return not IsValid(item.entity) end
  end,
      ...
      if char then
        ...
        if inv then
          ...
          if attData then
            ...
            local weightreduc = 0
            if targetitem.weightreduc then weightreduc = targetitem.weightreduc end
...
ITEM.functions.ModelOff = {
  ...
  tip = "useTip",
  icon = "icon16/wrench.png",
  OnCanRun = function(item) return not IsValid(item.entity) and item:GetData("equip") end,
  OnRun = function(item)
...
ITEM.functions.zCustomizeSkin = {
  ...
  OnCanRun = function(item) return not IsValid(item.entity) and #item.skincustom > 0 and item:GetData("equip") == true and item:GetOwner():GetCharacter():GetInventory():HasItem("paint") and item:GetOwner():GetCharacter():GetFlags("T") end,
  OnRun = function(item, data)
...
ITEM:Hook("drop", function(item)
  local client = item.player
  if item:GetData("equip") then item:RemoveOutfit(item:GetOwner()) end
end)
...
ITEM.functions.Equip = {
  ...
  OnRun = function(item)
    ...
    if item:GetData("setSkin", nil) ~= nil then client:SetSkin(item:GetData("setSkin", item.newSkin)) end
...
function ITEM:OnLoadout()
  if self:GetData("equip") then
    ...
    local character = client:GetCharacter()
    if self.newSkin then client:SetSkin(self.newSkin) end
    ...
    if self:GetData("setSkin", nil) ~= nil then client:SetSkin(self:GetData("setSkin", self.newSkin)) end
...
ITEM.functions.Inspect = {
  ...
  OnCanRun = function(item)
    local customData = item:GetData("custom", {})
    if not customData.img then return false end
...
ITEM.functions.Sell = {
  ...
  OnCanRun = function(item) return not IsValid(item.entity) and item:GetOwner():GetCharacter():HasFlags("1") end
}
