...
ix.command.Add("CharSetClass", {
  ...
  adminOnly = true,
  arguments = {ix.type.character, ix.type.string, ix.type.number, ix.type.number},
  OnRun = function(self, client, target, className, slot, level)
    if slot ~= 1 and slot ~= 2 and slot ~= 3 then return "You must assign the class to slot 1, 2, or 3 for Primary, Secondary, and Tertiary." end
    if slot == 1 then
      local Class = {}
      Class.level = level
      for k, v in pairs(ix.charclasses.list) do
        if ix.util.StringMatches(L(v.name, client), className) or ix.util.StringMatches(k, className) then
          if v.race then if v.race ~= target:GetBackground() then return "Target's Race cannot have this Class." end end
          Class.name = v.name
...
ix.command.Add("CharClearClass", {
  ...
  OnRun = function(self, client, target, slot)
    if slot ~= 1 and slot ~= 2 and slot ~= 3 then return "Slot must be 1, 2, or 3 for Primary, Secondary, and Tertiary class." end
    if slot == 1 then
...
function charMeta:HasClass(classname)
  ...
  if primary then if primary.name == classname then hasClass = primary.level end end
  if secondary then if secondary.name == classname then hasClass = secondary.level end end
  if tertiary then if tertiary.name == classname then hasClass = tertiary.level end end
  return hasClass
end
