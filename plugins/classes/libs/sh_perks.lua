...
function ix.charclasses.LoadFromDir(directory)
	for _, v in ipairs(file.Find(directory.."/*.lua", "LUA")) do
		...
		CHARCLASS = ix.charclasses.list[niceName] or {}
			if (PLUGIN) then
				CHARCLASS.plugin = PLUGIN.uniqueID
			end
...
-- Add updating of classes to the character metatable.
do
	local charMeta = ix.meta.character



	hook.Add("DoPluginIncludes", "ixClassesLib", function(path)
		ix.charclasses.LoadFromDir(path.."/charclasses")
	end)
end
