...
function PLUGIN:InitializedPlugins()
  for k, v in next, weapons.GetList() do
    ...
    if dat.Prim and not table.IsEmpty(dat.Prim) then
      for k2, v2 in next, dat.Prim do
        if v.Primary[k2] then v.Primary[k2] = v2 end
      end
    end

    if dat.Sec and not table.IsEmpty(dat.Sec) then
      for k2, v2 in next, dat.Sec do
        if v.Secondary[k2] then v.Secondary[k2] = v2 end
      end
    end
    ...
    if dat.iconCam then ITEM.iconCam = dat.iconCam end
    local atts = {}
    ...
    if dat.Weight then ITEM.Weight = dat.Weight end
    if dat.DurCh then ITEM.DurCh = dat.DurCh end
    ITEM:Hook("drop", function(item) item.player:EmitSound("physics/metal/metal_box_footstep1.wav") end)
        ...
        timer.Simple(0.2, function()
          if IsValid(wep) then
            for k, v in next, data do
              if self.Attachments[v] then wep:Attach(v) end
            end
          end
        end)
      end
    ...
    function ITEM:OnInstanced()
      if self:GetData("mods") == nil then self:SetData("mods", {}) end
    end
    ...
    ITEM.functions.detach = {
      ...
      OnCanRun = function(item) return not IsValid(item.entity) and IsValid(item.player) and item.invID == item.player:GetCharacter():GetInventory():GetID() and not table.IsEmpty(item:GetData("mods", {})) end,
      OnRun = function(item, data)
        if data and data[1] and data[2] then
          ...
          if IsValid(wep) then wep:Detach(data[2]) end
        end
        return false
      end,
    }
  ...
  for k, v in next, self.AttachData do
    ...
    if v.iconCam then ITEM.iconCam = v.iconCam end
      ...
      OnRun = function(item, data)
        if data and data[1] then
          ...
          if mods[item.slot] then
            ...
            if IsValid(wep) then wep:Attach(item.uniqueID) end
            return true
          end
        end
        return false
