...
if SERVER then ix.log.AddType("rollStat", function(client, attname, dicepool, pass, fail, rollstring, difficulty) return string.format("%s rolled for %s Difficulty %s with %s d10: %s \nSuccess: %s\nFail: %s", client:Name(), attname, difficulty, dicepool, rollstring, pass, fail) end) end
...
for k, v in pairs(ix.skills.list) do
  ix.command.Add(k, {
    ...
    OnRun = function(self, client, difficulty, modifier)
      ...
      local injurydebuff = char:GetHealthDebuff()
      if k == char:GetProfessionSkill() then professionboost = 1 end
      ...
      if modifier then modifierstring = "(Modifiier of " .. modifier .. ")" end
      ix.chat.Send(client, "rollStat", tostring(dicepool), nil, nil, {
        attname = attname,
        dicepool = dicepool,
        pass = pass,
...
ix.command.Add("fire", {
  ...
  OnRun = function(self, client, shots, difficulty, modifier)
    ...
    local professionboost = 0
    local injurydebuff = char:GetHealthDebuff()
    if char:GetProfessionSkill() == "marksmanship" then professionboost = 1 end
...
ix.command.Add("autofire", {
  ...
  OnRun = function(self, client, difficulty, rofmodifier, modifier)
    ...
    for _, group in ipairs(rollsplit) do
      ...
      if modifier then modifierstring = "(Modifier of " .. modifier .. ")" end
      ix.chat.Send(client, "rollStat", tostring(group), nil, nil, {
        attname = attname,
        dicepool = group,
        pass = pass,
...
function playerMeta:GetActiveWeaponItem()
  ...
  for k, v in pairs(self:GetChar():GetInv():GetItems()) do
    if v:GetData("equip", false) == true then if wepclass == v.class then weaponItem = v end end
  end
  return weaponItem
end
