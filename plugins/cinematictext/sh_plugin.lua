...
ix.config.Add("cinematicTextFont", "Arial", "The font used to display cinematic splash texts.", function() if CLIENT then hook.Run("LoadCinematicSplashTextFonts") end end, {
  category = PLUGIN.name
})

ix.config.Add("cinematicTextSize", 18, "The font size multiplier used by cinematic splash texts.", function() if CLIENT then hook.Run("LoadCinematicSplashTextFonts") end end, {
  category = PLUGIN.name,
  data = {
    min = 10,
    max = 50
...
ix.config.Add("cinematicTextSizeBig", 30, "The big font size multiplier used by cinematic splash texts.", function() if CLIENT then hook.Run("LoadCinematicSplashTextFonts") end end, {
  category = PLUGIN.name,
  data = {
    min = 10,
    max = 50
...
if CLIENT then
  ...
  function PLUGIN:LoadFonts()
    self:LoadCinematicSplashTextFonts()
  end
end
