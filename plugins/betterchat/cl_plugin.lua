...
local function GetTypingIndicator(text)
  ...
  if not prefix:find(symbolPattern) and text:utf8len() > 1 then
    return "ic"
  else
    local chatType = ix.chat.Parse(nil, text)
    if chatType and chatType ~= "ic" then return chatType end
    local start, _, commandName = text:find("/(%S+)%s")
    if start == 1 then
      for uniqueID, _ in pairs(ix.command.list) do
        if commandName == uniqueID then return uniqueID end
      end
    end
  end
end
...
function PLUGIN:PostDrawTranslucentRenderables()
  if not typing or not ix.option.Get("chatIndicators", true) then return end
  local radius = 0
