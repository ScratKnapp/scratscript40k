...
timer.Simple(30, function()
  ...
  ix.chat.Register("ic", {
    ...
    GetColor = function(self, speaker, text)
      ...
      if LocalPlayer():GetEyeTrace().Entity == speaker then color = ix.config.Get("chatListenColor") end
      return Flip(color)
    end,
  ...
  ix.command.Add("Event", {
    ...
    adminOnly = true,
    OnRun = function(self, client, text) ix.chat.Send(client, "event", text) end
  })

  ix.chat.Register("lev", {
    prefix = {"/EventLocal", "/Levent", "/Lev"},
    CanHear = ix.config.Get("chatRange", 280) * 8,
    OnChatAdd = function(self, speaker, text) chat.AddText(Color(205, 100, 0), text) end,
    indicator = "chatPerforming",
    CanSay = function(self, speaker, text) return speaker:IsAdmin() end
  })
end)
